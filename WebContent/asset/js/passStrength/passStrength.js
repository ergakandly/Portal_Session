jQuery(document).ready(function() {
		...
		(function($) {
			...
			var options = {
				...
				minChar : 8,
				scores : [ 17, 26, 40, 50 ],
				verdicts : [ "Password Strength : Weak", "Password Strength : Normal", "Password Strength : Medium", "Password Strength : Strong",
				...
				validationRules : {
					...
					wordLength : function(options, word, score) {
						...
						if (wordlen < options.minChar) {
							...
							options.errors
									.push(options.errorMessages.password_to_short);
						}
						return lenScore;
					},
					wordSimilarToUsername : function(options, word, score) {
						...
						if (username
							...
							options.errors
									.push(options.errorMessages.same_as_username);
							return score;
						}
						return true;
					},
					...
					return this
							.each(function(idx, el) {
								...
								if (allOptions.viewports.progress) {
									$(allOptions.viewports.progress).append(
											progressbar);
								} else {
								...
								if (allOptions.showVerdicts) {
									verdict = $('<span class="password-verdict">'
											+ allOptions.verdicts[0]
											+ '</span>');
									if (allOptions.viewports.verdict) {
										$(allOptions.viewports.verdict).append(
												verdict);
									} else {
										verdict.insertAfter($el);
									}
